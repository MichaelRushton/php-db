...
Runs before calling `PDO::connect`.
...
Runs after calling `PDO::connect`.
...
Runs before calling `PDO::prepare`.

```php
use MichaelRushton\DB\Events\BeforePrepareEvent;

$connection->beforePrepare(function (BeforePrepareEvent $event)
{
    $event->connection;
    $event->query;
});
...
Runs after calling `PDO::prepare`.

```php
use MichaelRushton\DB\Events\AfterPrepareEvent;

$connection->afterPrepare(function (AfterPrepareEvent $event)
{
    $event->connection;
    $event->query;
    $event->success;
...
Runs before calling `PDO::exec`, `PDO::query`, and `PDOStatement::execute`.

```php
use MichaelRushton\DB\Events\BeforeExecuteEvent;

$connection->beforeExecute(function (BeforeExecuteEvent $event)
{
    $event->connection;
    $event->query;
    $event->params;
...
Runs after calling `PDO::exec`, `PDO::query`, and `PDOStatement::execute`.

```php
use MichaelRushton\DB\Events\AfterExecuteEvent;

$connection->afterExecute(function (AfterExecuteEvent $event)
{
    $event->connection;
    $event->query;
    $event->success;
