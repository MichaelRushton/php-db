...
test("cache", function ()
{
  ...
  $connection1 = new Connection(Driver::SQLite, new PDO("sqlite:"));
  $connection2 = new Connection(Driver::SQLite, new PDO("sqlite:"));

  expect($connection1->prepare("SELECT ? AS c1"))
  ->toBe($connection1->prepare("SELECT ? AS c1"));

  expect($connection1->prepare("SELECT ? AS c1"))
  ->not->toBe($connection1->prepare("SELECT ? AS c2"));

  expect($connection1->prepare("SELECT ? AS c1"))
  ->not->toBe($connection2->prepare("SELECT ? AS c1"));
...
test("roll back transaction", function ()
{
  ...
  try
  {
    ...
    $connection->transaction(function ()
    {
      ...
      $this->query("INSERT INTO test DEFAULT VALUES");
